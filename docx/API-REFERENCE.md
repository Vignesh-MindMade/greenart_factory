...
| `variants[].gallery_url` | Points at `/gallery`, **a route that does not exist yet.** Do not link it until the gallery page ships. |
...
1. **`/gallery` does not exist.** `gallery_url` fields are inert strings.
